...
PR [#318](https://github.com/buildkite/terraform-provider-buildkite/pull/318) introduces a change to make it easier to use data-sources with Teams; both `slug` and `id` are now accepted as arguments. This change will be released in the next minor release. Only one of either `slug` or `id` should be set in order to use the `data.buildkite_team` data-source.
