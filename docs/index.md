...
- `max_retries` (Number) Maximum number of retry attempts for retryable HTTP requests. Defaults to 10. REST requests are also bounded by the operation timeout, which covers the waits between attempts as well as the requests themselves, so raising this value alone may not produce more attempts.
